"""
AudiobookSmith Book Processor v10 - Unified System
- AI chapter detection with TOC skipping (from v9)
- Proper chapter splitting with content allocation (from v9)
- ElevenLabs voice recommendations (from v7)
- Automatic voice sample generation (from v7)
...
If no clear chapters found, return total_chapters: 0 and empty chapters array."""
    ...
    def process(self):
        ...
        try:
            ...
            # Step 5.5: Split into chapter files (55-60%)
            tracker.update(55, "Splitting Chapters", "Creating individual chapter files...", eta_seconds=10)
            split_chapters = self.split_into_chapters(text, chapter_structure.get('chapters', []))
            tracker.update(60, "Chapters Split", f"Created {len(split_chapters)} chapter files", eta_seconds=15)
