...
class ElevenLabsVoiceRecommender:
    ...
    def get_available_voices(self, category: Optional[str] = None, narration_only: bool = True) -> List[Dict]:
        """
        Fetch available voices from ElevenLabs
        
        Args:
            category: Filter by category (professional, premade, etc.)
            narration_only: If True, only return voices suitable for narration
        
        Returns:
            List of voice dictionaries (filtered for narration if requested)
        """
        ...
        try:
            ...
            if narration_only:
                # Filter for narration-suitable voices only
                filtered_voices = []
                for voice in voices:
                    ...
                    # Prioritize narrative_story, but also include other suitable use cases
                    suitable_use_cases = [
                        'narrative_story',
                        'informative_educational',
                        'conversational'  # Can work for memoirs and personal stories
                    ]
                    
                    if any(uc in use_case for uc in suitable_use_cases):
                        filtered_voices.append(voice)
                
                print(f"   Filtered to {len(filtered_voices)} narration-suitable voices (from {len(voices)} total)")
